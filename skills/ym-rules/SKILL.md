...
- 默认不用 `var`；普通函数和回调偏好箭头函数。多条语句或控制流程使用代码块，有返回值时显式 `return`；简单表达式可使用表达式体。生成器、重载或需要动态 `this` 等情形按语言要求使用 `function`。
...
- 每个函数和数据声明都添加简短中文单行 `//` 注释：函数说明职责，数据说明用途或业务含义；不要使用 `/** */`。
- 优先使用 `const`，避免使用 `let`。
- 相邻的独立语句或声明之间空一行；模块顶层、函数体内及其他代码作用域都遵循此规则。不要把两条独立语句写在同一行；同一条多行语句内部不额外拆开。
- `if`、`else if` 和 `else` 的语句体一律使用花括号，即使只有一条语句也不省略。简单箭头函数可以使用表达式体省略花括号。
- 将错误处理、无效输入和退出条件放在函数前部，使用早返回减少嵌套。

### 数据声明注释

- 即使是简单字面量或语义明确的直接赋值，也为数据声明添加中文注释。
...
// 返回数值的两倍
const getDouble = (value: number) => value * 2;
```
